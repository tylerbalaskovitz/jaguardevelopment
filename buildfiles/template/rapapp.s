...
raptor_map_bitdepth				equ				4										; the bit depth of the tileset.  (Can be 1,2,4,8,16)
...
;;	
			...
			move.l	#0,raptor_mapbmptiles					; <<<<---- THIS WILL NEED TO BE CHANGED IF USING THE TILEMAP MODULE (see tilemap example) <<<<----
...
;;; Configure the tile map (these lines are commented out by default.)  Remove the ; if you are using Tile Maps)
;			
;			move.l	#raptor_first_map_object,raptor_maptop_obj
;			move.l	#0,raptor_map_position_x											; 16.16 value for X
;			move.l	#0,raptor_map_position_y											; 16.16 value for Y
;			move.l	#0,raptor_gmap_startx
;			move.l	#0,raptor_gmap_starty
;			move.l	#map1,raptor_mapindex												; tell RAPTOR where the tile list for the map is located		
;			move.l	#raptor_map_bitdepth,raptor_map_tiles_bits							; tell RAPTOR the bit depth of the tileset (1,2,4,8,16)
