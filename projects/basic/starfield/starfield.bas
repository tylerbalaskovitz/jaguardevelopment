' -----------------------------------------------------------------------
'
' Starfield particle effect demonstration
' sh3-rg 2015
'
' -----------------------------------------------------------------------
'
' How it works:		Multiple particle arrays are created and shown over 
'					various particle layers in the object list.
' -----------------------------------------------------------------------
...
DO
	...
	'Iterate over all the particle lists in turn and fire them
    IF counter=interval THEN
        rapParticleInject(strptr(particles))
    ENDIF    
    IF counter=interval*2   THEN
        rapParticleInject(strptr(particles2))
    ENDIF
    IF counter=interval*3   THEN
        rapParticleInject(strptr(particles3))
    ENDIF
    IF counter=interval*4   THEN
        rapParticleInject(strptr(particles4))
    ENDIF
    IF counter=interval*5   THEN
        rapParticleInject(strptr(particles5))
    ENDIF
    IF counter=interval*6   THEN
        rapParticleInject(strptr(particles6))
    ENDIF
    IF counter=interval*7   THEN
        rapParticleInject(strptr(particles7))
    ENDIF
    IF counter=interval*8   THEN
        rapParticleInject(strptr(particles8))
    ENDIF
    IF counter=interval*9   THEN
        rapParticleInject(strptr(particles9))
    ENDIF
    IF counter=interval*10  THEN
        rapParticleInject(strptr(particlesa))
    ENDIF
    IF counter=interval*11  THEN
        rapParticleInject(strptr(particlesb))
    ENDIF
    IF counter=interval*12  THEN
        rapParticleInject(strptr(particlesc))
    ENDIF
    IF counter=interval*13  THEN
        rapParticleInject(strptr(particlesd))
    ENDIF
    IF counter=interval*14  THEN
        rapParticleInject(strptr(particlese))
    ENDIF    
    IF counter=>interval*15 THEN
        rapParticleInject(strptr(particlesf))
        counter=0
    ENDIF
LOOP
