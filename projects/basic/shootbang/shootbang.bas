...
stars_gfx_loc=sprite[stars].gfxbase			                                ' R_sprite_gfxbase is the property of the sprite that says where the graphics live
                                                                            ...
                                                                            '   than that! :0)
stars_gfx_size=sprite[stars].framesz			                                ' R_sprite_framesz is this sprite property, we now know how large in memory a single
...
ship_gfx_loc=sprite[ship].gfxbase		                                    ' Memory location of ship sprite graphic.
ship_gfx_size=sprite[ship].framesz		                                    ' Memory size of each ship graphic, to be used as an offset to each frame.

' *******************************************************
' * SET UP COLOUR TABLE WITH PALETTES FROM ASSETS       *
' *******************************************************

jsfLoadClut(strptr(STARS_clut),10,16)                                       ' This pops the colours from the stars graphic into the last 16 of the CLUT
jsfLoadClut(strptr(SHIP_clut),14,16)                                        ' This pops the colours from the ship graphic into the 2nd to last 16 of the CLUT
jsfLoadClut(strptr(ENEMY_clut),13,16)                                       ' enemy colours
jsfLoadClut(strptr(BULLET_clut),12,16)                                      ' bullet colours
jsfLoadClut(strptr(BOOM_clut),11,16)                                        ' explosion colours

' *******************************************************
' * SET UP SOME STUFF BEFORE THE MAIN PROGRAM BEGINS    *
' *******************************************************

sprite[stars].gfxbase = stars_gfx_loc+(stars_gfx_size*5)			            ' Set the stars graphic to use the 5th frame of animation as a start point.
...
DO
    ...
    ship_pos = sprite[ship].x		                                        ' Read current ship x co-ordinate into the ship_pos variable.
    
    pad1=jsfGetPad(1)                                                          ' Read state of joypad 1 into pad1 variable.
...
SUB check_collision
    ...
    IF cd_flag>-1 THEN                                                                  ' -1 means *BAM* - *something* was it!
        ...
        FOR i=0 TO 4            
            check_for_hit_enemy=sprite[enemy1+i].was_hit			                    '   see if it is flagged as being hit (i.e.<>-1)
            IF check_for_hit_enemy<>-1 THEN     
                sprite[enemy1+i].was_hit = -1                               ' reset hit flag for enemy
                xunt=sprite[enemy1+i].x                                     '   read the x position of the enemy that was hit
                sprite[i+1[explosions]].x = xunt                            '   and set our explosion to spawn at the same x coordinate
                sprite[i+1[bigsplosions]].x = xunt-(16<<16)                 '   and set our big explosion to spawn 16 pixels left
                xunt=sprite[enemy1+i].y                                     '   read enemy y position of the enemy that was hit
                sprite[i+1[explosions]].y = xunt+(10<<16)                   '   and set our explosion to spawn at the enemy position + 10 in y
                randy=int(INT(RND*8))
                sprite[i+1[bigsplosions]].y = xunt-(randy<<16)              '   and set our explosion to spawn at the enemy position -6 in y
                sprite[i+1[explosions]].curframe = exp_max                  '   reset explosion to start (or we'll see nothing after 1st time!)
                sprite[i+1[bigsplosions]].curframe = exp_max                '   reset explosion to start (or we'll see nothing after 1st time!)
                sprite[i+1[explosions]].active = R_is_active                ' active the sprite
                sprite[i+1[bigsplosions]].active = R_is_active              ' active the sprite
            
				sprite[enemy1+i].scaled = R_spr_scale						' Set sprite to allow scaling (only do this when required!
				sprite[enemy1+i].scale_x = 48								' Enlarge the sprite on x
				sprite[enemy1+i].scale_y = 48								' Enarge the sprite on y
...
FOR i=0 TO 4
    ...
    sprite[enemy1+i].y = (60<<16)+((tucker+xunt)[sins]<<14)         ' <<14 instead of <<16 compresses the range of y values
NEXT i
...
SUB check_pad
    IF pad1 BAND JAGPAD_LEFT AND ship_pos>(86<<16) THEN
        sprite[ship].x = ship_pos-(2<<16)
        sprite[ship].gfxbase = ship_gfx_loc+ship_gfx_size
    ENDIF
    
    IF pad1 BAND JAGPAD_RIGHT AND ship_pos<(208<<16) THEN
        sprite[ship].x = ship_pos+(2<<16)
        sprite[ship].gfxbase = ship_gfx_loc+(ship_gfx_size*2)
    ENDIF
    
    IF pad1=0 OR pad1= JAGPAD_B THEN
        sprite[ship].gfxbase = ship_gfx_loc
    ENDIF
    
    IF pad1 BAND JAGPAD_B AND bullettime>=bulletdelay THEN
        bullettime=0
        sprite[bulletctr[bullets]].x = ship_pos+(9<<16)             ' set bullet to spawn at ship pos + offset to centre
        sprite[bulletctr[bullets]].y = (166<<16)                    ' set bullet to spawn at ship pos less offset to nose of ship
        sprite[bulletctr[bullets]].colchk = R_can_hit               ' reset flag stating this bullet can hit things
        sprite[bulletctr[bullets]].was_hit = -1                     ' reset flag to say the bullet has not collided
        sprite[bulletctr[bullets]].hitpoint = 10                   ' reset hitpoint value of bullet sprite to 10
        sprite[bulletctr[bullets]].active = R_is_active             ' active the sprite
        bulletctr++
