...
dim buffer[352*250*2/4] as integer							'define enough space to store the decompressed image
															...
															'the integer datatype uses 4 bytes within JagStudio and we do not need all 4 bytes of it, so we need to devide this by 4 so we dont waste space.
...
sprite[0].gfxbase = (int)buffer                             'change the object list graphic start address to the newly decompressed image (now stored in our buffer variable). 
															'we just need the start address of the variable so we just use the variable name
															
sprite[0].active = R_is_active                              'make the object visible       
