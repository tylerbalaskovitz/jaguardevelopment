...
DIM pad2 AS INTEGER							' Value to store the joypad movement in
...
Do
	...
	pad2 = jsfGetPad(RIGHT_PAD)				' Get the buttons pressed on the joypad

    IF pad2 BAND JAGPAD_UP THEN   			' If up is pressed, move the sprite up
        sprite[sprBug2].y_ -= 2       		' Subtract 2 from the y position
    ELSEIF pad2 BAND JAGPAD_DOWN THEN   	' If down is pressed, move the sprite down
        sprite[sprBug2].y_ += 2       		' Add 2 to the y position
    ENDIF

    IF pad2 BAND JAGPAD_LEFT THEN   		' If left is pressed, move the sprite left
        sprite[sprBug2].x_ -= 2       		' Subtract 2 from the x position
    ELSEIF pad2 BAND JAGPAD_RIGHT THEN   	' If right is pressed, move the sprite right
        sprite[sprBug2].x_ += 2       		' Add 2 to the x position
    ENDIF
